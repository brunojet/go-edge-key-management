...
func (s *RotationService) getPending(ctx context.Context, event RotationEvent) (*domain.SecretPayload, error) {
	payload, err := s.secrets.GetPending(ctx, event.SecretId, event.ClientRequestToken)
	if err != nil {
...
func (s *RotationService) getCurrentWithIntervalCheck(ctx context.Context, secretName string, minInterval time.Duration) (*domain.SecretPayload, error) {
	payload, err := s.secrets.GetCurrent(ctx, secretName)
	if err != nil {
